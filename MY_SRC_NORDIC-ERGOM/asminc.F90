MODULE asminc
   ...
   !!   trc_asm_inc    : Apply the BGC Ergom increment
   !!----------------------------------------------------------------------
...
#if defined key_top
   USE trc,       ONLY : trn, trb, tra, ln_ergom, ln_top_euler
   USE bioparam,  ONLY : idx_oxy, idx_nit, idx_amm, idx_phos, &
                         idx_dia, idx_flag, idx_cyano
#endif
   ...
   PUBLIC   trc_asm_inc    !: Apply the trc increment
#endif
   ...
   LOGICAL, PUBLIC :: ln_oxyinc     !: No oxygen concentration increment
   LOGICAL, PUBLIC :: ln_no3inc     !: No nitrate concentration increment
   LOGICAL, PUBLIC :: ln_nh4inc     !: No ammonium concentration increment
   LOGICAL, PUBLIC :: ln_po4inc     !: No phosphate concentration increment
   ...
   REAL(wp), PUBLIC :: nh4fac=1.0       !:Weight factor for bgc nh4 increment
   REAL(wp), PUBLIC :: po4fac=1.0       !:Weight factor for bgc po4 increment
   REAL(wp), PUBLIC :: flafac=1.0       !:Weight factor for bgc fla increment
   REAL(wp), PUBLIC :: diafac=1.0       !:Weight factor for bgc dia increment
   REAL(wp), PUBLIC :: cyafac=1.0       !:Weight factor for bgc cya increment
...
#endif
#if defined key_top && defined key_asminc
   REAL(wp), PUBLIC, DIMENSION(:,:,:)  , ALLOCATABLE ::   oxy_bkginc!, oxy_bkg 
   REAL(wp), PUBLIC, DIMENSION(:,:,:)  , ALLOCATABLE ::   no3_bkginc 
   REAL(wp), PUBLIC, DIMENSION(:,:,:)  , ALLOCATABLE ::   po4_bkginc 
   REAL(wp), PUBLIC, DIMENSION(:,:,:)  , ALLOCATABLE ::   nh4_bkginc
  ...
  integer, public :: n_update_bgc=0                    !< Number of BGC fields updates in the assimilation
  integer, public, allocatable :: ids_update_bgc(:)    !< Index array for BGC fields updated in the assimilation
  integer, public, allocatable :: fac_bgc(:)           !< Index array for BGC fields updated in the assimilation
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_asminc in configuration namelist', lwp )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Logical switch for applying bio tracer increments        ln_trcinc = ', ln_trcinc
#if defined key_top && key_asminc
#ifndef key_USE_PDAF
         IF (ln_trcinc) WRITE(numout,*) ' Logical switch for applying oxygen increment   ln_oxyinc = ', ln_oxyinc
         IF (ln_trcinc) WRITE(numout,*) ' Logical switch for applying ammonium increment ln_nh4inc = ', ln_nh4inc
         IF (ln_trcinc) WRITE(numout,*) ' Logical switch for applying nitrate increment  ln_no3inc = ', ln_no3inc
         IF (ln_trcinc) WRITE(numout,*) ' Logical switch for applying phosph increment   ln_po4inc = ', ln_po4inc
	 ...
	 WRITE(numout,*) '      Timestep of background in [0,nitend-nit000-1]            nitbkg    = ', nitbkg
      ...
      IF(lwp) THEN
         ...
         IF (ln_trcinc) WRITE(numout,*)' ditdinbgc_date  = ', ditdinbgc_date
         IF (ln_trcinc) WRITE(numout,*)' ditibgcstr_date = ', ditibgcstr_date
         IF (ln_trcinc) WRITE(numout,*)' ditibgcfin_date = ', ditibgcfin_date
      ENDIF 
...
#if defined key_top && defined key_asminc
      IF (ln_trcinc ) THEN
if (lwp)write (numout,*) 'Allocate BGC increment arrays'
         IF (ln_oxyinc) ALLOCATE( oxy_bkginc (jpi,jpj,jpk) ); oxy_bkginc   (:,:,:) = 0._wp
         IF (ln_no3inc) ALLOCATE( no3_bkginc (jpi,jpj,jpk) ); no3_bkginc   (:,:,:) = 0._wp
         IF (ln_nh4inc) ALLOCATE( nh4_bkginc (jpi,jpj,jpk) ); nh4_bkginc   (:,:,:) = 0._wp
         IF (ln_po4inc) ALLOCATE( po4_bkginc (jpi,jpj,jpk) ); po4_bkginc   (:,:,:) = 0._wp
         IF (ln_flainc) ALLOCATE( fla_bkginc (jpi,jpj,jpk) ); fla_bkginc   (:,:,:) = 0._wp
         IF (ln_cyainc) ALLOCATE( cya_bkginc (jpi,jpj,jpk) ); cya_bkginc   (:,:,:) = 0._wp
         IF (ln_diainc) ALLOCATE( dia_bkginc (jpi,jpj,jpk) ); dia_bkginc   (:,:,:) = 0._wp
      ENDIF
#endif
#if defined key_asminc
      ...
      IF ( ln_trainc .OR. ln_dyninc .OR.   &       !--------------------------------------
         ...
         ENDIF
         !
         !TO DO: z_inc_dateb und z_inc_datef anpassen und time anpassen und dann ln_trcinc aus 
         !       folgender If Bed. wieder rausnehmen
         IF ((.NOT. ln_trcinc) .AND.( ( z_inc_dateb < ndastp + nn_time0*0.0001_wp ) .OR.   & 
...
#if defined key_top && defined key_asminc
         IF ( ln_trcinc ) THEN
            IF (ln_oxyinc) CALL iom_get( inum, jpdom_autoglo, 'bckinoxy', oxy_bkginc, 1 )
            IF (ln_no3inc) CALL iom_get( inum, jpdom_autoglo, 'bckinno3', no3_bkginc, 1 )
            IF (ln_nh4inc) CALL iom_get( inum, jpdom_autoglo, 'bckinnh4', nh4_bkginc, 1 )
            IF (ln_po4inc) CALL iom_get( inum, jpdom_autoglo, 'bckinpo4', po4_bkginc, 1 )
            IF (ln_flainc) CALL iom_get( inum, jpdom_autoglo, 'bckinfla', fla_bkginc, 1 )
            IF (ln_cyainc) CALL iom_get( inum, jpdom_autoglo, 'bckincya', cya_bkginc, 1 )
            IF (ln_diainc) CALL iom_get( inum, jpdom_autoglo, 'bckindia', dia_bkginc, 1 )
            ! Apply the masks
            IF (ln_oxyinc) oxy_bkginc(:,:,:) = oxy_bkginc(:,:,:) * tmask(:,:,:)
            IF (ln_no3inc) no3_bkginc(:,:,:) = no3_bkginc(:,:,:) * tmask(:,:,:)
            IF (ln_nh4inc) nh4_bkginc(:,:,:) = nh4_bkginc(:,:,:) * tmask(:,:,:)
            IF (ln_po4inc) po4_bkginc(:,:,:) = po4_bkginc(:,:,:) * tmask(:,:,:)
            IF (ln_flainc) fla_bkginc(:,:,:) = fla_bkginc(:,:,:) * tmask(:,:,:)
            IF (ln_cyainc) cya_bkginc(:,:,:) = cya_bkginc(:,:,:) * tmask(:,:,:)
            IF (ln_diainc) dia_bkginc(:,:,:) = dia_bkginc(:,:,:) * tmask(:,:,:)
            ! Set missing increments to 0.0 rather than 1e+20
            ! to allow for differences in masks
            IF (ln_oxyinc) WHERE( ABS( oxy_bkginc(:,:,:) ) > 1.0e+10 ) oxy_bkginc(:,:,:) = 0.0
            IF (ln_no3inc) WHERE( ABS( no3_bkginc(:,:,:) ) > 1.0e+10 ) no3_bkginc(:,:,:) = 0.0
            IF (ln_nh4inc) WHERE( ABS( nh4_bkginc(:,:,:) ) > 1.0e+10 ) nh4_bkginc(:,:,:) = 0.0
            IF (ln_po4inc) WHERE( ABS( po4_bkginc(:,:,:) ) > 1.0e+10 ) po4_bkginc(:,:,:) = 0.0
            IF (ln_flainc) WHERE( ABS( fla_bkginc(:,:,:) ) > 1.0e+10 ) fla_bkginc(:,:,:) = 0.0
            IF (ln_diainc) WHERE( ABS( dia_bkginc(:,:,:) ) > 1.0e+10 ) dia_bkginc(:,:,:) = 0.0
            IF (ln_cyainc) WHERE( ABS( cya_bkginc(:,:,:) ) > 1.0e+10 ) cya_bkginc(:,:,:) = 0.0
         ENDIF
#endif
         !
         CALL iom_close( inum )
...
!#endif
 ...
 SUBROUTINE trc_asm_inc( kt )
      ...
      !!                    ***  ROUTINE trc_asm_inc  ***
      !!          
      !! ** Purpose : Apply the trc concentration assimilation increment.
      !!
      !! ** Method  : Direct initialization or Incremental Analysis Updating.
      !!
      !! ** Action  : 
   ...
   IF (ln_bgcdi) THEN
      IF ( (ln_top_euler .AND. ( kt == nitdinbgc_r )) .OR. &
         ...
         IF (ln_nh4inc)   tra(:,:,:,idx_amm) = tra(:,:,:,idx_amm)  + nh4_bkginc(:,:,:) * nh4fac
         IF (ln_po4inc)  tra(:,:,:,idx_phos) = tra(:,:,:,idx_phos) + po4_bkginc(:,:,:) * po4fac
         IF (ln_diainc)   tra(:,:,:,idx_dia) = tra(:,:,:,idx_dia)  + dia_bkginc(:,:,:) * diafac
         IF (ln_flainc)  tra(:,:,:,idx_flag) = tra(:,:,:,idx_flag) + fla_bkginc(:,:,:) * flafac
         IF (ln_cyainc) tra(:,:,:,idx_cyano) = tra(:,:,:,idx_cyano)+ cya_bkginc(:,:,:) * cyafac 
#endif
                ! 
      ENDIF
             !
   ELSEIF(ln_asmiau) THEN
      ...
      IF ( ( kt >= nitibgcstr_r ).AND.( kt <= nitibgcfin_r ) ) THEN
         ...
         ELSE
            zincwgt = 2.0D0*wgtiaubgc(it) !Factor 2 for Leap-frog time integration 
         ENDIF
         !
         IF(lwp) THEN
            WRITE(numout,*) 
         ...
         DO ibgc = 1, n_update_bgc
            i = ids_update_bgc(ibgc) ! index in bgc_bgkinc
            tra(:,:,:,i) = tra(:,:,:,i)  + BGC_bkginc(:,:,:,ibgc) * zincwgt
         END DO
#else
         IF (ln_oxyinc)   tra(:,:,:,idx_oxy) = tra(:,:,:,idx_oxy)  + oxy_bkginc(:,:,:) * zincwgt * oxyfac
         IF (ln_no3inc)   tra(:,:,:,idx_nit) = tra(:,:,:,idx_nit)  + no3_bkginc(:,:,:) * zincwgt * no3fac 
         IF (ln_nh4inc)   tra(:,:,:,idx_amm) = tra(:,:,:,idx_amm)  + nh4_bkginc(:,:,:) * zincwgt * nh4fac
         IF (ln_po4inc)  tra(:,:,:,idx_phos) = tra(:,:,:,idx_phos) + po4_bkginc(:,:,:) * zincwgt * po4fac
         IF (ln_diainc)   tra(:,:,:,idx_dia) = tra(:,:,:,idx_dia)  + dia_bkginc(:,:,:) * zincwgt * diafac
         IF (ln_flainc)  tra(:,:,:,idx_flag) = tra(:,:,:,idx_flag) + fla_bkginc(:,:,:) * zincwgt * flafac
         IF (ln_cyainc) tra(:,:,:,idx_cyano) = tra(:,:,:,idx_cyano)+ cya_bkginc(:,:,:) * zincwgt * cyafac 
#endif
            !
      ENDIF
