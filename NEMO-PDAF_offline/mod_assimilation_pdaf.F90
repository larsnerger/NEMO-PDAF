...
!< switch for using IAU or direct initialization
  logical :: ln_asmdin = .true.  ! using direct initialization in NEMO
  logical :: ln_asmiau = .false.    ! using IAU in nemo
...
contains
  ...
  ! from the external name!
    ...
    ! Interface between model and PDAF, and prepoststep
    external :: collect_state_pdaf, &  ! Collect a state vector from model fields
         prepoststep_ens_pdaf_offline  ! User supplied pre/poststep routine
  ...
  ! *** for the offline mode.                              ***
    ...
    IF (filtertype == 7) THEN
       if (mype_ens==0) write (*,'(1x,a)') 'Assimilate using OMI'

       CALL PDAFomi_put_state_local(collect_state_pdaf, init_dim_obs_pdafomi, &
            obs_op_pdafomi, prepoststep_ens_pdaf_offline, init_n_domains_pdaf, init_dim_l_pdaf, &
  ...
  ! ************************
    ...
    IF (status_pdaf /= 0) THEN
       WRITE (*,'(/1x,a6,i3,a47,i4,a1/)') &
            'ERROR ', status_pdaf, &
            ' during assimilation with PDAF - stopping! (PE ', mype_ens,')'
       CALL abort_parallel()
    END IF
