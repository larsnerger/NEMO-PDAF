...
!
!$Id: PDAF_diag_histogram.F90 374 2020-02-26 12:49:56Z lnerger $
!BOP
!
...
! 2021-11 - W.Düsterhöft-Wriggers - modified for state vectors including dry points
!
! !USES:
...
! ********************************
  ...
  IF (element > 0 .AND. element <= dim) THEN
     ...
     IF (.NOT.((state(element) > (0.0D0-eps)) .AND. (state(element)< (0.0D0+eps)))) THEN
  ...
  ELSE IF (element == 0) THEN
     ...
     DO elem = 1, dim
        ...
        IF (.NOT.((state(elem) > (0.0D0-eps)) .AND. (state(elem)< (0.0D0+eps)))) THEN
            hist(rank+1) = hist(rank+1) + 1
            countR = countR +1
        ENDIF
     END DO
...
! ***********************************************************
  ...
  IF (element > 0 .AND. element < dim) THEN
     print*,'Attention: not modified for state vectors with dry points'
  ...
  ELSE IF (element == 0) THEN
     ...
     DO elem = 1, dim_ens+1
        ...
        delta = delta + (hist(elem) - REAL(countR)/REAL(dim_ens+1))**2
     END DO
     !delta = delta * REAL(dim_ens+1) / REAL(ncall) / REAL(dim_ens)
     delta = ((delta)**0.5) / REAL (countR)
  ELSE
