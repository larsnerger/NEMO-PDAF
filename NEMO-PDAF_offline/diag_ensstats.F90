...
!
! !INTERFACE:
SUBROUTINE diag_ensstats(dim, dim_ens, element, &
  ...
  ELSE IF (element == 0) THEN
     ...
     DO elem = 1, dim
        ...
        IF (.NOT.((state(elem)<(0.0D0+eps)).AND.(state(elem)>(0.0D0-eps)))) THEN
           ...
           skewness = skewness + m3 / (sqrt(m2)**3 + eps2)
           kurtosis = kurtosis + m4 / (m2**2 + eps2) 
     ...
     !kurtosis = kurtosis / REAL(dim)
      skewness = skewness / REAL(countR)
      kurtosis = kurtosis / REAL(countR)
