...
subroutine init_ens_pdaf(filtertype, dim_p, dim_ens, state_p, Uinv, &
  ...
  use mod_assimilation_pdaf, &
       only:  type_ens_init, type_central_state, &
              ensscale, dim_state
  use mod_io_pdaf, &
...
! ********************************
  ...
  if (type_ens_init == 0) then
     ...
     call read_ens_mv_loop(trim(path_ens), dim_p, dim_ens, ens_p)
  ...
  elseif (type_ens_init == 2) then
     ...
     CALL gen_ens_mv(1.0_pwp, .true., trim(path_ens), dim_p, dim_ens, ens_p)
  ...
  elseif (type_ens_init == 3) then
     ...
     CALL gen_ens_from_cov(trim(path_ens)//trim(file_ens), dim_p, dim_ens, ens_p)
  ...
  if (type_central_state == 1) then
     ...
     call read_state_mv(path_inistate, dim_p, 1, state_p)
