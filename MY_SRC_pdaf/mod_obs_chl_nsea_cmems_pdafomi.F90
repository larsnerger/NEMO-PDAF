...
!!
!! Observation type: CHL observations for the North Sea from CMEMS
!! 
...
!!
module mod_obs_chl_nsea_cmems_pdafomi
  ...
  logical :: assim_chl_nsea_cmems = .false.  !< Whether to assimilate this data type
  real(pwp) :: rms_obs_chl_nsea_cmems = 0.3  !< Observation error standard deviation (for constant errors)
  real(pwp) :: lradius_chl_nsea_cmems = 1.0  !< Localization cut-off radius
  real(pwp) :: sradius_chl_nsea_cmems = 1.0  !< Support radius for weight function
  integer :: mode_chl_nsea_cmems = 0         !< Observation mode: 
                                        !< (0) linear interpolation
                                        !< (1) super-obbing: average 4 observation values
  integer :: time_chl_nsea_cmems = 0         !< Time at which the daily observations are assimilated
                                        !< (0) mightnight, (12) noon
  character(len=3) :: dist_chl_nsea_cmems = 'geo'  ! Type of distance computation: 
  ...
  character(lc) :: path_chl_nsea_cmems = '.' !< Path to observation file
  character(lc) :: file_chl_nsea_cmems = ''  !< Filename for observations
  character(lc) :: varname_chl_nsea_cmems = 'CHL'  !< Name of observation variable in file
...
!!
  subroutine init_dim_obs_chl_nsea_cmems(step, dim_obs)
...
! *********************************************
    ...
    if (mype_filter==0) &
         write (*,'(a,4x,a)') 'NEMO-PDAF', 'Assimilate observations - OBS_CHL_NSEA_CMEMS'
    ...
    if (assim_chl_nsea_cmems) thisobs%doassim = 1

    ! Specify type of distance computation
    if (trim(dist_chl_nsea_cmems) == 'gp') then
...
! **********************************
    ...
    doassim: if (doassim_now) then
       ...
       ! The CHL data can can be downloaded as follows:
       ! python -m motuclient --motu https://my.cmems-du.eu/motu-web/Motu \
       !    --service-id OCEANCOLOUR_BAL_BGC_L3_MY_009_133-TDS \
       !    --product-id mems_obs-oc_bal_bgc-plankton_my_l3-multi-1km_P1D \
       !    --longitude-min -4.5 --longitude-max 30.5 --latitude-min 48.5 --latitude-max 66 \
       !    --date-min "2015-01-01 00:00:00" --date-max "2015-01-31 23:59:59" \
       !    --variable CHL \
       !    --out-name chl_ba_MY_201501.nc --user <USERNAME> --pwd <PASSWD>

       ! read observation values and their coordinates
       file_full = trim(path_chl_nsea_cmems)//trim(file_chl_nsea_cmems)

       if (mype_filter==0) then 
          write (*,'(a, 4x,a,i3,a)') 'NEMO-PDAF', 'Read observations for day ', &
               iday,' from file:'
          write (*,'(a, 4x,a)') 'NEMO-PDAF', trim(file_full)
          write (*, '(a,4x,a,a)') 'NEMO-PDAF', '--- name of CHL file variable: ', trim(varname_chl_nsea_cmems)
       end if
       ...
       call check( nf90_inq_varid(ncid, trim(varname_chl_nsea_cmems), varid) )
       ...
       cartdist: if (trim(dist_chl_nsea_cmems) == 'gp') then
...
! ***********************************************************
       ...
       obsmodeA: if (mode_chl_nsea_cmems==0 .and. obsgrid_p) then
       ...
       if (npes_filter==1) then
          write (6,'(a, 4x, a, i7)') 'NEMO-PDAF', '--- number of observations from CHL_NSEA_CMEMS: ', dim_obs
       else
          if (screen>2) then
             write (6,'(a, 4x, a, i4, 2x, a, i7)') 'NEMO-PDAF', 'PE', mype_filter, &
                  '--- number of observations from CHL_NSEA_CMEMS: ', dim_obs
          end if
       end if
    ...
    if (dim_obs_p > 0) then
       ...
       obsmodeB: if (mode_chl_nsea_cmems==0) then
             ...
             do i = ido_w, ido_e
                ...
                haveobs: if (obs_from_file(i,j) > missing_value) then
                   ...
                   wetpoint: if (idx_nwet(idm_w, idm_n)>0 .and. idx_nwet(idm_e, idm_n)>0 .and. &
                      ...
                      obs_p(cnt) = real(obs_from_file(i, j))
       ...
       if (mode_chl_nsea_cmems==0) allocate(thisobs%icoeff_p(4, 1))
    ...
    if (sfields(id_chl)%transform == 2) then
       obs_p(:) = log(obs_p) - 0.5d0 * rms_obs_chl_nsea_cmems*rms_obs_chl_nsea_cmems*ln10sq
    end if
...
! ****************************************************************
    ...
    if (sfields(id_chl)%transform == 2) then
       ...
       var_obs =  rms_obs_chl_nsea_cmems*rms_obs_chl_nsea_cmems*ln10sq
       ...
       var_obs =  rms_obs_chl_nsea_cmems*rms_obs_chl_nsea_cmems
...
! ****************************************
    ...
    call PDAFomi_gather_obs(thisobs, dim_obs_p, obs_p, ivar_obs_p, ocoord_p, &
         thisobs%ncoord, lradius_chl_nsea_cmems, dim_obs)
...
! ********************
  ...
  end subroutine init_dim_obs_chl_nsea_cmems
...
!!
  subroutine obs_op_chl_nsea_cmems(dim_p, dim_obs, state_p, ostate)
...
! ******************************************************
  ...
  end subroutine obs_op_chl_nsea_cmems
...
!!
  subroutine init_dim_obs_l_chl_nsea_cmems(domain_p, step, dim_obs, dim_obs_l)
...
! **********************************************
    ...
    call PDAFomi_init_dim_obs_l(thisobs_l, thisobs, coords, &
         locweight, lradius_chl_nsea_cmems, sradius_chl_nsea_cmems, dim_obs_l)

  end subroutine init_dim_obs_l_chl_nsea_cmems
...
!!
  subroutine localize_covar_chl_nsea_cmems(dim_p, dim_obs, HP_p, HPH, coords_p)
...
! *************************************
    ...
    call PDAFomi_localize_covar(thisobs, dim_p, locweight, lradius_chl_nsea_cmems, sradius_chl_nsea_cmems, &
         coords_p, HP_p, HPH)

  end subroutine localize_covar_chl_nsea_cmems

end module mod_obs_chl_nsea_cmems_pdafomi
