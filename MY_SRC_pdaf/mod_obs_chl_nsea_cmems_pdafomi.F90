...
module mod_obs_chl_nsea_cmems_pdafomi
  ...
  real(pwp) :: omit_chl_nsea_cmems = 0.0     !< Omit obs. if innovation > obs. rms * this factor
...
! *** Variables used inside the module
  integer, private :: observation_mode            !< Observation mode (alias for mode_chl_nsea_cmems)
  character(len=3) :: dist_obs                    !< Type of distance computation (alias for dist_chl_nsea_cmems)
  character(lc) :: obsname = 'OBS_CHL_NSEA_CMEMS' !< Observation name string
...
! *********************************************
    ...
    region = 'no'
    limcoords(1) = 56.8 * deg2rad    ! north/south limit in Skagerrak
    ...
    dist_obs = dist_chl_nsea_cmems           ! Type of distance computation
    observation_mode = mode_chl_nsea_cmems   ! Whether to use interpolation or super-obbing
    varname_lon = 'longitude'                ! Name of longitude variable in file
    varname_lat = 'latitude'                 ! Name of latitude variable in file
    varname_obs = varname_chl_nsea_cmems     ! Name of observation variable in file
    rms_obs = rms_obs_chl_nsea_cmems         ! Obs. error standard deviation
    missing_value = -999.0                   ! Missing value in observation file
    file_full = trim(path_chl_nsea_cmems)//trim(file_chl_nsea_cmems)   ! File name including path
    ...
    thisobs%inno_omit = omit_chl_nsea_cmems
